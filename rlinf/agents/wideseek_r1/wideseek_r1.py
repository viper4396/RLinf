...
class WideSeekR1AgentLoopWorker(MultiAgentLoopWorker):
    ...
    async def compute_token_logprobs(
        ...
        target_logprobs = (
            raw_logprobs[-n_target:-1]
            if len(raw_logprobs) >= n_target
            else raw_logprobs
        )
        # Pad / truncate to exact length.
        if len(target_logprobs) < len(target_token_ids):
            target_logprobs = [0.0] * (
                len(target_token_ids) - len(target_logprobs)
            ) + target_logprobs
    ...
    async def run_one_query(self, prompt_ids: list[int], *, answer) -> AgentLoopOutput:
        ...
        skip_llm_judge = reward_mode == "trajectory" and advantage_mode == "trajectory"
        ...
        if max_response_len > length_limit:
            t_frac = (max_response_len - length_limit) / (
                max_length_limit - length_limit
            )
        ...
        traj_reward_agg_pre = (
            ...
            if (succ_end and answer_format_ok)
            else 0.0
        )
        ...
        if (
            ...
            llm_turn_rewards = await evaluate_turn_rewards(
                ...
                num_planner_turns=len(num_turn_subagents) if num_turn_subagents else 0,
            )
    ...
    def get_rollout_metrics(
        ...
        rollout_batch = {
            ...
            "outcome_reward_metric": rollout_result.extra_fields_traj["outcome_reward"],
