...
def _compute_tool_call_metrics(
    ...
    _add_weighted_mean_metric(
        metrics,
        "wideseek_r2/traj/mean/subtask",
        sum(traj_subtask_counts),
        num_trajectories,
    )
    _add_weighted_mean_metric(
        metrics,
        "wideseek_r2/traj/mean/search",
        sum(traj_search_counts),
        num_trajectories,
    )
    _add_weighted_mean_metric(
        metrics,
        "wideseek_r2/traj/mean/access",
        sum(traj_access_counts),
        num_trajectories,
    )
    _add_weighted_mean_metric(
        metrics,
        "wideseek_r2/turn/mean/subtask",
        sum(turn_subtask_counts),
        num_valid_planner_turns,
    )
    _add_weighted_mean_metric(
        metrics,
        "wideseek_r2/turn/mean/search",
        sum(turn_search_counts),
        num_valid_worker_turns,
    )
    _add_weighted_mean_metric(
        metrics,
        "wideseek_r2/turn/mean/access",
...
def _compute_mas_turn_metrics(rollout_batch: dict) -> dict:
    ...
    _add_weighted_mean_metric(
        metrics,
        "wideseek_r2/avg_main_agent_turns_per_traj",
        sum_main_turns,
        num_valid_trajs,
    )
    _add_weighted_mean_metric(
        metrics,
        "wideseek_r2/avg_subagent_turns_per_traj",
        sum_subagent_turns,
        num_valid_trajs,
    )
    _add_weighted_mean_metric(
        metrics,
        "wideseek_r2/avg_num_subagents_per_traj",
        sum_num_subagents,
        num_valid_trajs,
    )
    return metrics
...
def _compute_final_answer_format_metrics(rollout_batch: dict) -> dict:
    ...
    _add_weighted_mean_metric(
        metrics,
        "wideseek_r2/traj/mean/final_answer_format",
        sum(values),
        len(values),
    )
    return metrics
