...
from rlinf.agents.wideseek_r2.utils.webpage import WebPageCache
...
class WideSeekR2ToolWorker(ToolWorker):
    """Tool worker that serves `search` and `access` requests for WideSeek-R2."""
    ...
    def __init__(self, cfg: DictConfig):
        ...
        if self.use_online_search:
            self.log_info(
                "[INFO] WideSeekR2ToolWorker: Using online search (Serper API)"
            )
            self.search_client = AsyncOnlineSearchClient(cfg=self.cfg)
        else:
            self.log_info("[INFO] WideSeekR2ToolWorker: Using local search server")
    ...
    async def _process_requests(self):
        ...
        async def generate_and_send(channel_key: str, tool_name: str, tool_args: dict):
            ...
            try:
                ...
                self.log_error(
                    f"[ERROR] WideSeekR2ToolWorker: Tool execution failed for {tool_name}: {e}, tool name is {tool_name}, tool args is {tool_args}"
                )
                result = ToolChannelResponse(
                    success=False,
...
if __name__ == "__main__":
    """Test WideSeekR2ToolWorker with both online and offline modes."""
    import argparse

    from omegaconf import OmegaConf

    # Parse command line arguments
    parser = argparse.ArgumentParser(description="Test WideSeekR2ToolWorker")
    parser.add_argument(
        "--is_online",
    ...
    print("=" * 60)
    print(
        f"Testing WideSeekR2ToolWorker - {'ONLINE' if args.is_online else 'OFFLINE'} mode"
    )
    print("=" * 60)
